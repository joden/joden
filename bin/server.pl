...
use DateTime;
	...
	sub daemonize
	{
		...
		open(STDOUT, '>>/opt/joden/logs/access.log') or die("Unable to write logs: $!");
		open(STDERR, '>>/opt/joden/logs/error.log')  or die("Unable to write logs: $!");
	...
	{
		my($_pid) = shift;
			open(PID, '>>/opt/joden/logs/pid.log');
				print(PID "$_pid\n");
			close(PID);
	}
